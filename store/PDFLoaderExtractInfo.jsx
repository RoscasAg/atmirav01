...
const PDFLoader = () => {
  ...
  const handleSubmitPrompt = async (endpoint) => {
    try {
      ...
      setMessages((prevMessages) => [
        ...prevMessages,
        {
          text: searchRes.result.answer,
          //text: searchRes.result.output,
          type: "bot",
          sourceDocuments: searchRes.result.context,
        },
      ]);
      notificationCtx.showNotification({
        title: "Success!",
  ...
  const handleSubmitPromptDB = async (endpoint) => {
    try {
      notificationCtx.showNotification({
        title: "Loading...",
        message: "Please wait while we process your request!",
  ...
  const handleInfoExtraction = async () => {
    try {
      notificationCtx.showNotification({
        ...
        message: "Please wait while we process your PDF!",
        status: "pending",
      });

      if (!state.opanaillama) {
        // Using the llamar_parse API
        const response = await fetch(
          "https://atmira-neo4j.onrender.com/process-pdf/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            ...
            body: JSON.stringify({
              pdf_file_path: selectedPdfPath,
              customer_id: nif,
              customer_name: "Miquel Rivera",
            }),
          },
        );
        if (!response.ok) {
        ...
        notificationCtx.showNotification({
          title: "PDF Processed",
          message: "PDF has been successfully processed!",
          status: "success",
        });
      } else {
        // Using the OpenAI API
        const response = await fetch(
          "https://atmira-neo4j-openai.onrender.com/process-pdf/",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      const graphResponse = await fetch(`/api/populateGraph_v2`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          nif, // Assuming you want to pass the NIF and the result of the PDF processing
          //...result, // Spread operator to include all properties of the result object
        }),
      });
  ...
  const handleSubmitdatapdf = () => {
    if (state.datapdf) {
      handleSubmitPrompt("/pdf-query");
    } else {
      handleSubmitPromptDB("/db-query");
    }
  };
  ...
  return (
    ...
    >
      ...
      <PromptBox
        ...
        handleSubmit={handleSubmitdatapdf}
        placeHolderText="Ask something about your information?"
        error={error}
      />
    </Box>
  );
